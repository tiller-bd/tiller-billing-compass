...
async function GET(request) {
    try {
        ...
        if (categoryId && categoryId !== "all") where.categoryId = parseInt(categoryId); // Added logic
        if (year && year !== "all") {
            where.startDate = {
        ...
        if (status && status !== "all") {
            filtered = projects.filter((p)=>{
                const isPaid = p.bills.length > 0 && p.bills.every((b)=>b.status === "PAID");
...
async function POST(request) {
    try {
        ...
        const { projectName, clientId, newClient, departmentId, categoryId, startDate, endDate, totalProjectValue, bills } = body;
                ...
                data: {
                    ...
                    bills: {
                        create: bills.map((bill)=>({
                                ...
                                tentativeBillingDate: bill.tentativeBillingDate ? new Date(bill.tentativeBillingDate) : null,
                                status: "PENDING"
                            }))
                    }
                }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__649e7662._.js.map